var requestOptions = {
  ...
  fetch('http://localhost:5000/persona', requestOptions)
  .then(res => {
    if (!res.ok) {
  ...
  .then(json => {
    document.getElementById("name").textContent = "Hola soy: " + json.nombre + " " + json.lastname;
    ...
    document.getElementById("website").textContent = "Visitame en mi blog: " + json.blog;
  })
